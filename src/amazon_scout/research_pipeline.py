...
from .scoring import load_scoring_config, opportunity_score
...
def _component_freshness(records: list[EvidenceRecord], as_of: datetime, config: dict[str, Any]) -> str:
    ...
    for state in (EvidenceFreshness.CURRENT, EvidenceFreshness.AGING, EvidenceFreshness.STALE):
...
def canonical_funnel(raw_funnel: dict[str, Any], analyses: list[dict[str, Any]]) -> dict[str, int]:
    ...
    validated = min(evidence_backed, sum(a["validated_opportunity_score"] is not None for a in analyses))
    finalists = min(validated, sum(a["recommendation_tier"] == "VALIDATED_CANDIDATE" for a in analyses))
    bundles = sum(a["candidate_type"] == "BUNDLE_HYPOTHESIS" for a in analyses)
    return {"generated": generated, "screened": screened, "web_evidence_backed": evidence_backed, "evidence_backed": evidence_backed, "serpapi_validated": serpapi_validated, "price_gate_passed": sum(a["gates"]["price"]["gate"] for a in analyses), "demand_gate_passed": sum(a["gates"]["demand"]["gate"] for a in analyses), "competition_gate_passed": sum(a["gates"]["competition"]["gate"] for a in analyses), "risk_gate_passed": sum(a["gates"]["risk"]["gate"] for a in analyses), "validated": validated, "bundle_hypotheses": bundles, "finalists": finalists}
...
def analyze_evidence_bundle(raw: dict[str, Any], records: list[EvidenceRecord], *, generated_at: datetime | None = None) -> list[dict[str, Any]]:
    ...
    for niche in sorted(set(by_niche) | set(products_by_niche)):
        ...
        margin_score = round(clamp(100 - known_fee / fee_basis * 180), 2) if known_fee is not None and fee_basis else None
        ...
        results.append({
            ...
            "known_fee_components": ["referral fee"] if known_fee is not None else [], "unknown_fee_components": ["FBA fulfillment fee", "fee VAT", "storage/advertising/returns"], "fee_scenarios": scenarios,
