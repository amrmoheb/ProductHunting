    ...
    with db.connect() as connection:
        ...
        for item in analyses:
            ...
            if item["preliminary_opportunity_score"] is not None:
                connection.execute("INSERT INTO opportunity_scores(run_id,niche,score,confidence_score,factors_json,scoring_version,calculated_at,preliminary_score,validated_score,recommendation_tier) VALUES(?,?,?,?,?,?,?,?,?,?)", (run_id,item["niche"],item["preliminary_opportunity_score"],item["data_confidence_score"],json.dumps(item["factors"]),item.get("scoring_version","V1.4D"),now,item["preliminary_opportunity_score"],item["validated_opportunity_score"],item["recommendation_tier"]))
