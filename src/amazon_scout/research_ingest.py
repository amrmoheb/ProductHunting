...
def ingest(path: str | Path, database: str | Path = "data/scout.db", *, quarantine_future: bool = False, slug_suffix: str = "", additional_evidence: str | Path | None = None) -> tuple[Path, Path, int]:
    ...
    with db.connect() as connection:
        ...
        for item in analyses:
            ...
            for metric, value in (("demand_score", item["demand_score"]), ("competition_score", item["competition_score"]), ("risk_score", item["risk_score"]), ("preliminary_opportunity_score", item["preliminary_opportunity_score"]), ("validated_opportunity_score", item["validated_opportunity_score"]), ("data_confidence_score", item["data_confidence_score"])):
            ...
            if item["preliminary_opportunity_score"] is not None:
                connection.execute("INSERT INTO opportunity_scores(run_id,niche,score,confidence_score,factors_json,scoring_version,calculated_at,preliminary_score,validated_score,recommendation_tier) VALUES(?,?,?,?,?,?,?,?,?,?)", (run_id,item["niche"],item["preliminary_opportunity_score"],item["data_confidence_score"],json.dumps(item["factors"]),"research-v1.1",now,item["preliminary_opportunity_score"],item["validated_opportunity_score"],item["recommendation_tier"]))
