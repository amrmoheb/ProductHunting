...
from .production_scoring_v14d import PROPOSED_OPPORTUNITY_WEIGHTS
...
def _has_reportable_economics(item: dict[str, Any]) -> bool:
    econ=item.get("economics") or {}; score=econ.get("score") or {}; base=(econ.get("scenarios") or {}).get("BASE") or {}
    raw=score.get("raw") if isinstance(score,dict) else score
    return isinstance(raw,(int,float)) and not isinstance(raw,bool) or any(isinstance(base.get(key),(int,float)) and not isinstance(base.get(key),bool) for key in ("selling_price_aed","total_amazon_operating_cost_before_product_aed")) or any(isinstance(value,(int,float)) and not isinstance(value,bool) for value in (base.get("maximum_landed_cost_aed") or {}).values())


def _economics_section(analyses: list[dict[str, Any]]) -> list[str]:
    economics_weight=PROPOSED_OPPORTUNITY_WEIGHTS["economics"]
    items=[item for item in analyses if _has_reportable_economics(item)]
    ...
    for item in items:
        econ=item.get("economics",{}); category=econ.get("category",{}); scenarios=econ.get("scenarios",{}); base=scenarios.get("BASE",{}); stored_score=econ.get("score",{}); score=stored_score if isinstance(stored_score,dict) else {"raw":stored_score}; profile=econ.get("physical_profile") or {}; sens=econ.get("sensitivity",{})
        final_score=float(item.get("validated_opportunity_score") or item.get("preliminary_opportunity_score") or 0)
        economics_row=next((row for row in (item.get("score_breakdown") or {}).get("arithmetic",[]) if row.get("component")=="economics"),None)
        contribution=float(economics_row["contribution"]) if economics_row and isinstance(economics_row.get("contribution"),(int,float)) else float(score.get("raw") or 0)*economics_weight
        before=round(final_score-contribution,2)
...
def render_research_report(raw: dict[str, Any], analyses: list[dict[str, Any]], source_status: dict[str, str] | None = None, *, generated_at: datetime | None = None) -> str:
    ...
    if raw.get("v13_economics") or any(_has_reportable_economics(item) for item in analyses):
