...
- DataForSEO is optional, but its current Amazon Labs keyword/ranking datasets are documented as US/English-only. The adapter therefore refuses to substitute those figures for UAE. Merchant API locations/languages must be discovered before any future UAE call.
