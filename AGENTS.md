...
Paid providers are forbidden unless `RESEARCH_ALLOW_PAID_PROVIDERS=true`, `RESEARCH_MAX_PAID_CALLS` permits the batch, and `RESEARCH_MAX_COST_USD` covers its estimated cost. Stop before exceeding either limit. SerpApi may be used with `amazon_domain=amazon.ae`. DataForSEO Amazon UAE location 2784 has partial Arabic Amazon-source coverage and is supplemental only; English Amazon Labs coverage remains NOT_CONFIRMED. Product Competitors and Ranked Keywords are optional competition evidence only when observed. Rainforest remains fail-closed until current official amazon.ae support is confirmed.
...
9. Use the canonical V1.4D production scorer for the deterministic opportunity score. Never invent or manually overwrite it. Weights live in `config/scoring.yaml`: demand 30%, competition attractiveness 25%, unchanged V1.3 economics 35%, risk attractiveness 10%. Missing evidence contributes zero with no favorable redistribution; confidence remains separate.
...
Research-mode gates: FINAL_TOP_10 requires multiple current observations plus price, competition, demand, and risk evidence. Confidence 55–69 is capped at VALIDATED; confidence below 55 is PRELIMINARY_NEEDS_EVIDENCE; STRONG requires confidence >=70, score threshold, known risk, and adequate economics. Never multiply opportunity by confidence or force a Top 10.
